...
github = 'https://raw.githubusercontent.com/gabzcastillo03/Moon-Aery/main/Dataset/Combined%20Testing/PageRank_Dataset.csv'
...
def vowel_con_ratio(raw_words):
    vowels = sum(1 for char in raw_words if char.casefold() in 'aeiou')
    consonants = sum(1 for char in raw_words if char.casefold() in 'qwrtypsdfghjklzxcvbnm')
    if consonants == 0:
...
def host_entropy(hostname):
    """Calculates the Shannon entropy of a string in subdomain"""
    freqs = Counter(hostname)
    probs = [float(freqs[c]) / len(hostname) for c in freqs]
    entropy = -sum([p * math.log(p) / math.log(2.0) for p in probs])
    return entropy
...
def extract_features(url):
    ...
    features["vowel_con_ratio"] = vowel_con_ratio(raw_words)
    ...
    features["host_entropy"] = host_entropy(hostname)
...
df1 = pd.DataFrame(df['url'].apply(extract_features).tolist())
df1 = pd.concat([df1, df[['url', 'PageRank', 'status']].reset_index(drop=True)], axis=1)

df1.to_csv(r'D:\Downloads\superduper_final_features.csv', index = False)
